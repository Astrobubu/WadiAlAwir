...
export default async function AdminInvoicesPage({ searchParams }: { searchParams: Promise<{ q?: string }> }) {
  ...
  return <>
    <div className="admin-page-heading"><div><p className="admin-eyebrow">Sales</p><h1>Invoices</h1><p>{invoices.length} invoices connected to the same products and inventory.</p></div>{canCreate && <Link href="/admin/invoices/new" className="admin-button admin-button--primary"><AdminIcon name="plus" /> New invoice</Link>}</div>
