...
export default function AdminStockItemForm({ item = {} }: { item?: StockItemValue }) {
  ...
  return <form action={action} className="admin-editor admin-stock-form"><input type="hidden" name="id" value={item.id ?? ''} />{state.error && <p className="admin-form-error" role="alert">{state.error}</p>}
    {item.isCatalogue && <div className="admin-info-strip"><div><strong>Storefront product</strong><span>This inventory record belongs to a published or draft product. Public names, descriptions and images remain editable from its full product editor.</span></div></div>}
    <section className="admin-editor__section"><div className="admin-editor__section-heading"><span>01</span><div><h2>Product details</h2><p>Used in the unified Products list and invoice picker.</p></div></div><div className="admin-form-grid admin-form-grid--3"><Field label="Name" required><input name="name" defaultValue={item.name} required /></Field><Field label="SKU"><input name="sku" defaultValue={item.sku ?? ''} /></Field><Field label="Category"><input name="category" defaultValue={item.category ?? ''} placeholder="Body, Lighting, Service…" /></Field><Field label="Unit"><select name="unit" defaultValue={item.unit ?? 'pcs'}><option value="pcs">Pieces</option><option value="set">Set</option><option value="m">Meters</option><option value="l">Liters</option><option value="kg">Kilograms</option></select></Field></div></section>
    <section className="admin-editor__section"><div className="admin-editor__section-heading"><span>02</span><div><h2>Inventory and pricing</h2><p>Margin and inventory value update immediately after saving.</p></div></div><div className="admin-form-grid admin-form-grid--2"><Field label="Quantity"><input name="quantity" type="number" min="0" step="0.001" defaultValue={item.quantity ?? 0} /></Field><Field label="Low-stock alert"><input name="low_stock_threshold" type="number" min="0" step="0.001" defaultValue={item.low_stock_threshold ?? 0} /></Field><Field label="Cost price (AED)"><input name="cost_price" type="number" min="0" step="0.01" defaultValue={item.cost_price ?? 0} /></Field><Field label="Selling price (AED)"><input name="selling_price" type="number" min="0" step="0.01" defaultValue={item.selling_price ?? 0} /></Field></div><Field label="Supplier notes"><textarea name="supplier_notes" rows={4} defaultValue={item.supplier_notes ?? ''} /></Field></section>
    <footer className="admin-editor__footer"><span>{item.id ? 'A history snapshot is saved before this update.' : 'Invoice-only items stay off the public storefront.'}</span><button className="admin-button admin-button--primary" disabled={pending}>{pending ? 'Saving…' : item.id ? 'Save changes' : 'Add item'}</button></footer>
  </form>
}
